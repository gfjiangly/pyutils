...
def split_data(root, files, dst, test_size=0.1):
    data_list = pyutils.read_files_to_list(root, files)
    train_list, test_list = split_list(data_list, test_size)
    # import time
    # now = time.strftime('%Y-%m-%d-%H-%M-%S', time.localtime(time.time()))
    pyutils.write_list_to_file(train_list, dst+'_train.txt')
    pyutils.write_list_to_file(test_list, dst+'_test.txt')
    return train_list, test_list
...
def makedirs(path):
    ...
    Update: 2020.5.29
    Notes:
        传入的path是文件名。如果是路径，请直接使用os.makedirs
    """
